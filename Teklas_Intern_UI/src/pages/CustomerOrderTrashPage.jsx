...
const CustomerOrderTrashPage = () => {
  ...
  const handleRestore = async (item) => {
    ...
    const result = await MySwal.fire({
      ...
      text: `Bu müşteri siparişini geri yüklemek istediğinizden emin misiniz?`,
      icon: 'question',
    ...
    if (result.isConfirmed) {
      ...
      try {
        await axios.post(`${BASE_URL}/api/customerorder/${item.id}/restore`);
        await fetchDeletedData();
        MySwal.fire({
          title: 'Geri Yüklendi!',
          text: 'Müşteri siparişi başarıyla geri yüklendi.',
          icon: 'success',
  ...
  const handlePermanentDelete = async (item) => {
    ...
    const result = await MySwal.fire({
      ...
      text: 'Bu müşteri siparişini kalıcı olarak silmek istediğinizden emin misiniz? Bu işlem geri alınamaz!',
      icon: 'warning',
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      setRestoreLoading(true);
      try {
        await axios.delete(`${BASE_URL}/api/customerorder/${item.id}/permanent`);
        await fetchDeletedData();
        MySwal.fire({
          title: 'Kalıcı Olarak Silindi!',
          text: 'Müşteri siparişi kalıcı olarak silindi.',
          icon: 'success',
          customClass: { popup: isDark ? 'swal2-dark' : '' },
        });
      } catch (err) {
  ...
  const dataWithRowNumber = data.map((item, idx) => ({
    ...
    customerName: item.customerName || 'Müşteri yok',
      ...
      <div className="col-lg-12">
        <div className="card h-100">
          <div className="card-header d-flex justify-content-between align-items-center">
            <h5 className="card-title mb-0">Silinen Müşteri Siparişleri</h5>
...
export default CustomerOrderTrashPage; 