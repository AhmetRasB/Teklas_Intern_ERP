...
const ProductionConfirmationTrashPage = () => {
  ...
  const fetchDeletedData = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${BASE_URL}/api/production/confirmation/deleted`);
      console.log('Deleted Production Confirmation data:', res.data); // Debug log
      setData(res.data);
    } catch (err) {
      console.error('Error fetching deleted Production Confirmation data:', err); // Debug log
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleRestore = async (item) => {
    ...
    const result = await MySwal.fire({
      ...
      text: `Bu üretim teyidini geri yüklemek istediğinizden emin misiniz?`,
      icon: 'question',
    ...
    if (result.isConfirmed) {
      ...
      try {
        await axios.put(`${BASE_URL}/api/production/confirmation/${item.confirmationId}/restore`);
        await fetchDeletedData();
        MySwal.fire({
          title: 'Geri Yüklendi!',
          text: 'Üretim teyidi başarıyla geri yüklendi.',
          icon: 'success',
  ...
  const handlePermanentDelete = async (item) => {
    ...
    const result = await MySwal.fire({
      ...
      text: 'Bu üretim teyidini kalıcı olarak silmek istediğinizden emin misiniz? Bu işlem geri alınamaz!',
      icon: 'warning',
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      setRestoreLoading(true);
      try {
        await axios.delete(`${BASE_URL}/api/production/confirmation/${item.confirmationId}/permanent`);
        await fetchDeletedData();
        MySwal.fire({
          title: 'Kalıcı Olarak Silindi!',
          text: 'Üretim teyidi kalıcı olarak silindi.',
          icon: 'success',
          customClass: { popup: isDark ? 'swal2-dark' : '' },
        });
      } catch (err) {
  ...
  const columns = [
    ...
    { header: 'Teyit Tarihi', accessor: 'confirmationDate', render: val => val ? new Date(val).toLocaleString('tr-TR') : '' },
    { header: 'Üretilen Miktar', accessor: 'quantityProduced' },
    { header: 'Yapan Kişi', accessor: 'performedBy' },
    { header: 'Silme Tarihi', accessor: 'deleteDate', render: val => val ? new Date(val).toLocaleString('tr-TR') : '' },
  ];
  ...
  const dataWithRowNumber = data.map((item, idx) => ({ 
    ...
    deleteDate: item.deleteDate || item.updateDate || 'Tarih mevcut değil'
  }));
      ...
      <div className="col-lg-12">
        <div className="card h-100">
          <div className="card-header d-flex justify-content-between align-items-center">
            <h5 className="card-title mb-0">Silinen Üretim Teyitleri</h5>
...
export default ProductionConfirmationTrashPage; 