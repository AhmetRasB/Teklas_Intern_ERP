...
const MaterialCategoryTrashPage = () => {
  ...
  const fetchDeletedData = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${BASE_URL}/api/material/category/deleted`);
      console.log('Deleted Material Category data:', res.data); // Debug log
      setData(res.data);
    } catch (err) {
      console.error('Error fetching deleted Material Category data:', err); // Debug log
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleRestore = async (item) => {
    ...
    const result = await MySwal.fire({
      ...
      text: `Bu malzeme kategorisini geri yüklemek istediğinizden emin misiniz?`,
      icon: 'question',
    ...
    if (result.isConfirmed) {
      ...
      try {
        await axios.put(`${BASE_URL}/api/material/category/${item.id}/restore`);
        await fetchDeletedData();
        MySwal.fire({
          title: 'Geri Yüklendi!',
          text: 'Malzeme kategorisi başarıyla geri yüklendi.',
          icon: 'success',
  ...
  const handlePermanentDelete = async (item) => {
    ...
    const result = await MySwal.fire({
      ...
      text: 'Bu malzeme kategorisini kalıcı olarak silmek istediğinizden emin misiniz? Bu işlem geri alınamaz!',
      icon: 'warning',
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      setRestoreLoading(true);
      try {
        await axios.delete(`${BASE_URL}/api/material/category/${item.id}/permanent`);
        await fetchDeletedData();
        MySwal.fire({
          title: 'Kalıcı Olarak Silindi!',
          text: 'Malzeme kategorisi kalıcı olarak silindi.',
          icon: 'success',
          customClass: { popup: isDark ? 'swal2-dark' : '' },
        });
      } catch (err) {
  ...
  const columns = [
    ...
    { header: 'Kategori Kodu', accessor: 'categoryCode' },
    { header: 'Kategori Adı', accessor: 'categoryName' },
      ...
      <div className="col-lg-12">
        <div className="card h-100">
          <div className="card-header d-flex justify-content-between align-items-center">
            <h5 className="card-title mb-0">Silinen Malzeme Kategorileri</h5>
