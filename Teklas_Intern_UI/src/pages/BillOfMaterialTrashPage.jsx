...
const BillOfMaterialTrashPage = () => {
  ...
  const fetchDeletedData = async () => {
    ...
    try {
      ...
      console.log('Deleted BOM data:', res.data); // Debug log
      setData(res.data);
    } catch (err) {
      console.error('Error fetching deleted BOM data:', err); // Debug log
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleRestore = async (item) => {
    ...
    const result = await MySwal.fire({
      ...
      text: `Bu BOM kaydını geri yüklemek istediğinizden emin misiniz?`,
      icon: 'question',
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        MySwal.fire({
          title: 'Geri Yüklendi!',
          text: 'BOM kaydı başarıyla geri yüklendi.',
          icon: 'success',
  ...
  const handlePermanentDelete = async (item) => {
    ...
    const result = await MySwal.fire({
      ...
      text: 'Bu BOM kaydını kalıcı olarak silmek istediğinizden emin misiniz? Bu işlem geri alınamaz!',
      icon: 'warning',
      showCancelButton: true,
    ...
    if (result.isConfirmed) {
      setRestoreLoading(true);
      try {
        await axios.delete(`${BASE_URL}/api/production/bom/${item.bomHeaderId}/permanent`);
        await fetchDeletedData();
        MySwal.fire({
          title: 'Kalıcı Olarak Silindi!',
          text: 'BOM kaydı kalıcı olarak silindi.',
          icon: 'success',
          customClass: { popup: isDark ? 'swal2-dark' : '' },
        });
      } catch (err) {
      ...
      <div className="col-lg-12">
        <div className="card h-100">
          <div className="card-header d-flex justify-content-between align-items-center">
            <h5 className="card-title mb-0">Silinen Ürün Ağaçları (BOM)</h5>
...
export default BillOfMaterialTrashPage; 