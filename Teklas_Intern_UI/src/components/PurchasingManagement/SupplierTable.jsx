...
const SupplierTable = () => {
  ...
  const fetchData = () => {
    ...
    axios.get(`${BASE_URL}/api/suppliers?${params}`)
      .then(res => {
  ...
  const fetchDeletedData = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/api/suppliers/deleted`);
  ...
  const handleRestore = async (item) => {
    try {
      await axios.put(`${BASE_URL}/api/suppliers/${item.id || item.Id}/restore`);
      fetchDeletedData();
      fetchData();
      MySwal.fire({
        title: 'Başarılı!',
        text: 'Tedarikçi başarıyla geri alındı.',
        icon: 'success',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        await axios.put(`${BASE_URL}/api/suppliers/${form.id}`, form);
        MySwal.fire({
          title: 'Başarılı!',
  ...
  const handleDelete = async (item) => {
    const result = await MySwal.fire({
      title: 'Emin misiniz?',
      text: `"${item.name || item.Name}" adlı tedarikçiyi silmek istediğinize emin misiniz?`,
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${BASE_URL}/api/suppliers/${item.id || item.Id}`);
        fetchData();
        MySwal.fire({
          title: 'Başarılı!',
  ...
  const handleEdit = (row) => {
    setForm({
      ...
      SupplierTypeId: row.supplierTypeId || row.SupplierTypeId || ''
    });
    setEditMode(true);
    setShowModal(true);
  };
  ...
  const columns = [
    ...
    { header: "Tedarikçi Tipi", accessor: "supplierTypeName" },
    ...
    <div className="col-lg-12">
      <div className="card h-100">
        <div className="card-header d-flex justify-content-between align-items-center">
          <h5 className="card-title mb-0">Tedarikçiler</h5>
          <div className="d-flex gap-2 align-items-center">
            ...
            <button
              ...
              onClick={() => navigate('/supplier-trash')}
            >
        ...
        <SupplierModal
          ...
          loading={formLoading}
          error={formError}
          title={editMode ? 'Tedarikçi Düzenle' : 'Tedarikçi Ekle'}
        ...
        {showDetail && (
          <div
            ...
            onClick={() => { setShowDetail(false); setSelectedSupplier(null); }}
          >
            <style>{`
            ...
            >
              <div className="modal-content" style={{ borderRadius: 16, padding: '24px' }}>
                <div className="modal-header">
                  <h5 className="modal-title">Tedarikçi Detay</h5>
                  <button type="button" className="btn-close" onClick={() => { setShowDetail(false); setSelectedSupplier(null); }}></button>
                </div>
                <div className="modal-body" style={{padding: '16px 8px 8px 8px'}}>
                  <div style={{maxHeight: '70vh', overflowY: 'auto', padding: 0}}>
                    {selectedSupplier && (
                      <div>
                        {[
                          ['Ad', selectedSupplier.name],
                          ['Adres', selectedSupplier.address],
                          ['Telefon', selectedSupplier.phone],
                          ['E-posta', selectedSupplier.email],
                          ['Vergi Numarası', selectedSupplier.taxNumber],
                          ['İletişim Kişisi', selectedSupplier.contactPerson],
                          ['Tedarikçi Tipi', selectedSupplier.supplierTypeName],
                          ['Aktif mi?', selectedSupplier.isActive ? 'Aktif' : 'Pasif'],
                          ['Oluşturulma', formatDateTime(selectedSupplier.createdDate)],
                          ['Güncellenme', formatDateTime(selectedSupplier.updatedDate)],
                          ['Oluşturan', selectedSupplier.createdBy],
                          ['Güncelleyen', selectedSupplier.updatedBy],
                        ].map(([label, value], i) => (
                ...
                <div className="modal-footer d-flex justify-content-end gap-2" style={{background: 'transparent', borderTop: 'none', boxShadow: 'none', padding: 0}}>
                  <button className="btn btn-soft-primary" onClick={() => handleEdit(selectedSupplier)}>Düzenle</button>
                  <button className="btn btn-soft-danger" onClick={() => handleDelete(selectedSupplier)}>Sil</button>
                  <button className="btn btn-secondary" onClick={() => { setShowDetail(false); setSelectedSupplier(null); }}>Kapat</button>
                </div>
              </div>
            </div>
          </div>
