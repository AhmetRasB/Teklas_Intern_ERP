...
const SupplierTypeTable = () => {
  ...
  const fetchData = () => {
    setLoading(true);
    axios.get(`${BASE_URL}/api/suppliertype`)
      .then(res => {
        setData(res.data);
        setLoading(false);
      })
  ...
  const handleRestore = async (item) => {
    setRestoreLoading(true);
    try {
      await axios.post(`${BASE_URL}/api/suppliertype/${item.id || item.Id}/restore`);
      fetchDeletedData();
      fetchData();
      MySwal.fire({ title: 'Geri Alındı!', text: 'Tedarikçi tipi başarıyla geri alındı.', icon: 'success' });
    } catch {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode && form.id) {
        await axios.put(`${BASE_URL}/api/suppliertype/${form.id}`, payload);
      } else {
        await axios.post(`${BASE_URL}/api/suppliertype`, payload);
      }
      setShowModal(false);
      setEditMode(false);
      setForm(initialForm);
  ...
  const handleDelete = async (item) => {
    ...
    const result = await MySwal.fire({
      title: 'Silme Onayı',
      text: 'Bu tedarikçi tipini silmek istediğinize emin misiniz?',
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${BASE_URL}/api/suppliertype/${item.id || item.Id}`);
        fetchData();
        MySwal.fire({
          title: 'Silindi!',
          text: 'Tedarikçi tipi başarıyla silindi.',
          icon: 'success',
        ...
        <div className="card-header d-flex justify-content-between align-items-center">
          ...
          <div className="d-flex gap-2 align-items-center">
            ...
            <button
              ...
              onClick={() => navigate('/supplier-type-trash')}
            >
        ...
        <div className="card-body">
          <div className="table-responsive">
            <TableDataLayer
              ...
              actions={canWrite('purchasingManagement') ? ['edit', 'delete'] : []}
            />
          </div>
        </div>
        <SupplierTypeModal
          ...
          loading={formLoading}
          error={formError}
          title={editMode ? 'Tedarikçi Tipi Düzenle' : 'Tedarikçi Tipi Ekle'}
        />
      </div>
    </div>
  );
