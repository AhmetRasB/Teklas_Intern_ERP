...
@media (max-width: 900px) {
  .supplier-modal-grid { grid-template-columns: 1fr !important; }
}
`;
...
const validationSchema = yup.object().shape({
  Name: yup.string().required('Tedarikçi adı zorunludur.').max(100, 'Tedarikçi adı en fazla 100 karakter olabilir.'),
  ...
  SupplierTypeId: yup.number().required('Tedarikçi tipi zorunludur.'),
});
    ...
    <div style={modalOverlayStyle}>
      ...
      <div style={{...a4LandscapeStyle, ...modalBg}} className="a4-landscape-modal">
        ...
        <Form onSubmit={handleSubmit} style={{flex: 1, overflow: 'auto'}}>
          ...
          <div style={gridStyle} className="supplier-modal-grid">
            <div>
            ...
            <div>
              ...
              <Form.Group className="mb-3">
                <Form.Label>Tedarikçi Tipi *</Form.Label>
                <Form.Select name="SupplierTypeId" value={form.SupplierTypeId || ''} onChange={onChange} required isInvalid={!!validationErrors.SupplierTypeId} style={inputStyles}>
                  <option value="">Tedarikçi Tipi Seçin</option>
                  {supplierTypes.map(type => (
                    <option key={type.id} value={type.id}>{type.name}</option>
                  ))}
                </Form.Select>
                {validationErrors.SupplierTypeId && <Form.Control.Feedback type="invalid">{validationErrors.SupplierTypeId}</Form.Control.Feedback>}
              </Form.Group>
            </div>
          </div>
