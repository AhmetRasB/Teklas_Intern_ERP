...
@media (max-width: 1200px) {
  .purchase-order-modal-grid { grid-template-columns: 1fr 1fr !important; }
}
@media (max-width: 800px) {
  .purchase-order-modal-grid { grid-template-columns: 1fr !important; }
}
`;
...
const validationSchema = yup.object().shape({
  ...
  supplierId: yup.string().required('Tedarikçi seçimi zorunludur.'),
    ...
    <div style={modalOverlayStyle}>
      ...
      <div style={{...a4LandscapeStyle, ...modalBg, background: isDark ? '#273142' : modalBg.background}} className="a4-landscape-modal">
        ...
        <Form onSubmit={handleSubmit} style={{flex: 1, overflow: 'auto'}}>
          ...
          <div style={gridStyle} className="purchase-order-modal-grid">
            <div>
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Select name="supplierId" value={form.supplierId} onChange={onChange} required isInvalid={!!validationErrors.supplierId} style={inputStyles} disabled={isDetail}>
                  <option value="">Seçiniz</option>
                  {suppliers.map(sup => <option key={sup.id} value={sup.id}>{sup.name}</option>)}
                </Form.Select>
                {validationErrors.supplierId && <Form.Control.Feedback type="invalid">{validationErrors.supplierId}</Form.Control.Feedback>}
              </Form.Group>
              <Form.Group className="mb-3">
