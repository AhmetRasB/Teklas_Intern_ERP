...
const TABLE_KEY = 'CustomerOrderTable';
...
const CustomerOrderTable = () => {
  ...
  const fetchData = () => {
    setLoading(true);
    let url = `${BASE_URL}/api/customerorder`;
  ...
  const fetchDeletedData = async () => {
    ...
    try {
      const res = await axios.get(`${BASE_URL}/api/customerorder/deleted`);
  ...
  const handleRestore = async (item) => {
    setRestoreLoading(true);
    try {
      await axios.post(`${BASE_URL}/api/customerorder/${item.id || item.Id}/restore`);
      fetchDeletedData();
      fetchData();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode && form.id) {
        await axios.put(`${BASE_URL}/api/customerorder/${form.id}`, payload);
      } else {
        await axios.post(`${BASE_URL}/api/customerorder`, payload);
      }
      setShowModal(false);
      setEditMode(false);
      setForm(initialForm);
  ...
  const handleDelete = async (item) => {
    ...
    const result = await MySwal.fire({
      title: 'Silme Onayı',
      text: 'Bu müşteri siparişini silmek istediğinize emin misiniz?',
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${BASE_URL}/api/customerorder/${item.id || item.Id}`);
        fetchData();
        MySwal.fire({
          title: 'Silindi!',
          text: 'Müşteri siparişi başarıyla silindi.',
          icon: 'success',
    ...
    <div className="col-lg-12">
      <div className="card h-100">
        <div className="card-header d-flex justify-content-between align-items-center">
          <h5 className="card-title mb-0">Müşteri Siparişleri</h5>
          <div className="d-flex gap-2 align-items-center">
            ...
            <button
              ...
              onClick={() => navigate('/customer-order-trash')}
            >
        ...
        <div className="card-body">
          <div className="table-responsive">
            <TableDataLayer
              ...
              actions={canWrite('salesManagement') ? ['edit', 'delete'] : []}
            />
          </div>
        </div>
        <CustomerOrderModal
